...
public class PlaylistReader
{
        ...
        FileNotFoundException
    {
            ...
            && playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            if (tokens.length < 8)
            {
                ...
                throw new ParseException(
                    "not enough values; needs to have 8",
                    PLAYLIST_TOKENS - tokens.length);
            }
            else if (tokens.length > 8)
            {
                // 2b.TODO: throw corresponding exception, along with a message
                throw new ParseException(
                    "too many values; needs to have 8",
                    PLAYLIST_TOKENS - tokens.length);
            }
            ...
            else
            {
                    ...
                    || !isInValidPercentRange(maxPop, maxRock, maxCountry))
                {
                    throw new DailyMixDataException(
                        "genre set values are not within range");
                }
        ...
        if (playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            throw new DailyMixDataException("needs to have 3 playlists");
        }
        ...
        FileNotFoundException
    {
        ...
        while (file.hasNextLine())
        {
                ...
                && tokens.length != SONG_TOKENS - 1)
            {
                ...
                throw new ParseException(
                    "incorrect number of tokens; needs to have 5",
                    SONG_TOKENS - tokens.length);
            }
    ...
    {
        return PlaylistCalculator.MIN_PERCENT <= num1
            && num1 <= PlaylistCalculator.MAX_PERCENT
            && PlaylistCalculator.MIN_PERCENT <= num2
            && num2 <= PlaylistCalculator.MAX_PERCENT
            && PlaylistCalculator.MIN_PERCENT <= num3
